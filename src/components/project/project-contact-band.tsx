...
interface Props {
  ...
  projectTitle: string
  agent:        AgentInfo
  /** Optional location (project address / canton) shown on the bottom
   *  "Our location" card. */
...
/**
 * Editorial contact band for project pages — same warm cream surface
 * as the rest of the page (no dark inversion). Layout matches the
 * Affan-style reference: text rail left, form card right, decorative
 * wordmark behind, 3 contact cards at the bottom.
 */
export function ProjectContactBand({
  projectId, projectTitle, agent, locationLabel,
}: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!name.trim() || !email.trim()) {
      toast.error("Escribí tu nombre y correo para que el asesor te contacte.")
      return
    }
    startTransition(async () => {
      ...
      toast.success("¡Mensaje enviado! El asesor se pondrá en contacto pronto.")
      setDone(true)
    })
  }

  // WhatsApp prefill — strip non-digits.
  const waNumber = agent.phone?.replace(/[^\d]/g, "") ?? null
  const waText   = encodeURIComponent(
    `Hola, me interesa el proyecto "${projectTitle}". ¿Podemos hablar?`,
  )
  ...
  return (
    ...
    >
      {/* Decorative "Contacto" backdrop wordmark — very subtle on cream
          so it reads as typographic texture, not copy. */}
      <div
          ...
          <div className="lg:col-span-5 space-y-(--spacing-block)">
            <div className="space-y-(--spacing-cluster)">
              ...
              <p className="text-sm sm:text-base text-muted-foreground leading-relaxed max-w-md">
                {projectTitle ? (
                  <>¿Te interesa <span className="font-semibold text-foreground">{projectTitle}</span>? Dejanos tus datos y un asesor acreditado te contacta para resolver dudas y agendar la visita.</>
                ) : (
                  <>Dejanos tus datos y un asesor acreditado te contacta para resolver dudas y agendar la visita.</>
                )}
              </p>
            </div>
            ...
            {done ? (
              <div className="rounded-3xl bg-card ring-1 ring-foreground/5 shadow-sm p-(--spacing-section) text-center space-y-(--spacing-cluster)">
                ...
                <p className="text-sm text-muted-foreground">
                  {agent.full_name ?? "Un asesor"} te contactará pronto.
                </p>
              </div>
            ) : (
              <form
              ...
              >
                ...
                <FormTextarea
                  placeholder={`Hola, me interesa "${projectTitle}"…`}
                  rows={6}
                  value={message}
