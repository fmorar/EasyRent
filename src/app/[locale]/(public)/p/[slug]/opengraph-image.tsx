...
import { formatListingState } from "@/lib/labels"
import type { MarketplaceProperty } from "@/types"

/**
 * Dynamic Open Graph image for public property pages.
 *
 * Next.js evaluates this file as a special route handler — the
 * generated PNG is wired into `<head>` automatically as `og:image`
 * (and `twitter:image`) for the colocated route.
 *
 * Composition (1200×630):
 *   • Cover photo as full-bleed background (object-fit: cover)
 *   • Bottom gradient → readable text overlay
 *   • Bottom-left block: state chip, title, price, specs
 *   • Top-right pill: easyrent wordmark (always visible → brand
 *     stays present even when the property photo dominates the
 *     preview, e.g. WhatsApp / Telegram link cards)
 *
 * Fallback (no cover photo): solid foreground card with the wordmark
 * centered and the property fact line below. Still branded.
 *
 * The image is generated server-side via Satori. Satori has two
 * non-obvious requirements:
 *   1. Every container with children must set `display: "flex"`
 *   2. External image fetches must succeed at request time — we
 *      catch failures and fall back to the branded card.
 */

export const alt         = "Vista previa de propiedad en easyrent"
...
// Brand tokens — kept literal because Satori does not run Tailwind.
const FOREGROUND = "#0A0A0A"
const BACKGROUND = "#FAFAF7"
const MUTED      = "#6B7280"
const ACCENT     = "#FACC15"

export default async function PropertyOgImage({ params }: Props) {
  const { slug } = await params
  const supabase = await createClient()

  // Pull the same row + property_photos used by the page itself so the
  // preview matches the live listing exactly.
  const { data: property } = await supabase
    .from("v_marketplace")
    .select(
      "id, title, listing_type, status, property_type, price, currency, bedrooms, bathrooms, area_sqm, display_address",
    )
    .eq("slug", slug)
    .single() as {
      data: Pick<
        MarketplaceProperty,
        | "id" | "title" | "listing_type" | "status" | "property_type"
        | "price" | "currency" | "bedrooms" | "bathrooms" | "area_sqm"
        | "display_address"
      > | null
    }

  // Cover photo — `is_cover` wins; otherwise the first photo by order.
  ...
  if (property?.id) {
    ...
    if (photos && photos.length > 0) {
      coverUrl = photos.find((p) => p.is_cover)?.url ?? photos[0]!.url
    }
  }

  // Pre-formatted strings. Keep them short — long titles wrap, but
  // anything past ~80 chars starts to crowd the price line.
  const title        = property?.title?.trim() || "Propiedad en easyrent"
  const stateLabel   = property
    ? formatListingState(property.listing_type ?? "rent", property.status ?? "available")
    : "easyrent"
  ...
  const specsLabel   = [
    ...
    property?.bathrooms != null ? `${property.bathrooms} bn` : null,
    property?.area_sqm  != null ? `${property.area_sqm} m²` : null,
  ].filter(Boolean).join(" · ")
  const addressLabel = property?.display_address?.trim() || null
  ...
  return new ImageResponse(
      ...
      >
        {coverUrl ? (
          ...
          <>
            ...
            >
              <Wordmark color={FOREGROUND} height={28} />
            </div>

            {/* Accent bar at bottom for brand */}
            <div
              style={{
                position:   "absolute",
                left:       0,
            ...
            >
              ...
              <div
                style={{
                  ...
                  // Clamp very long titles so the OG card never overflows.
                  maxWidth:   1080,
                }}
              >
                {clamp(title, 90)}
              ...
              >
                ...
                {(specsLabel || addressLabel) && (
                  ...
                  >
                    {[specsLabel, addressLabel].filter(Boolean).join(" · ")}
                  </div>
                )}
              </div>
            </div>
          ...
          >
            <div style={{ display: "flex", alignItems: "center" }}>
              <Wordmark color={FOREGROUND} height={56} />
            </div>
    ...
    {
      ...size,
      // Note: we deliberately don't load custom fonts here. Satori's
      // built-in fallback handles the Latin character set fine for our
      // copy (Spanish + Catalan accents) and keeps cold-start fast.
    },
  )
}
...
/**
 * Inline wordmark for Satori. Mirrors `<EasyrentLogo>` paths but
 * accepts an explicit color (Satori does not resolve `currentColor`
 * inside SVG, so we wire it via the `fill` attribute on each path).
 */
