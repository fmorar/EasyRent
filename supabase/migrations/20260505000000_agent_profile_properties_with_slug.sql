-- Add `slug` to get_agent_profile_properties so callers can build /p/<slug>
-- canonical URLs instead of falling back to UUIDs. Postgres can't change
-- function return types via REPLACE so we drop and recreate.
...
AS $function$
  SELECT
    ...
    p.status, p.bedrooms, p.bathrooms, p.area_sqm, p.floor, p.display_address,
